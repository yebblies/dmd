...
public:
    ...
    MATCH deduceFunctionTemplateMatch(TemplateInstance ti, Scope* sc, ref FuncDeclaration fd, Type tthis, Expressions* fargs)
    {
        ...
        Lmatch: for (size_t i = 0; i < dedtypes.dim; i++)
        {
            ...
            if (!oarg)
            {
                ...
                else
                {
                    ...
                    if (!oded)
                    {
                        ...
                        if (tp && fptupindex == IDX_NOTFOUND && ntargs == dedargs.dim - 1)
                        {
    ...
    RootObject declareParameter(Scope* sc, TemplateParameter tp, RootObject o)
    {
         ...
         */
        if (v)
            return cast(RootObject)v._init.toExpression();
        return o;
    }
    ...
    public:
        ...
        void visit(TypeFunction t)
        {
            ...
            L1:
                ...
                L2: for (size_t i = 0; i < nfparams; i++)
                {
...
public:
    ...
    MATCH matchArg(Scope* sc, RootObject oarg, size_t i, TemplateParameters* parameters, Objects* dedtypes, Declaration* psparam)
    {
        ...
        if (!ei && oarg)
        {
            ...
            FuncDeclaration f;
            if (si && (f = si.isFuncDeclaration()) !is null)
            {
                ei = new VarExp(loc, f);
                ei = ei.semantic(sc);
                if (!f.needThis())
...
public:
    ...
    void semantic(Scope* sc, Expressions* fargs)
    {
        ...
        assert(tempdecl);
        if (Module m = tempdecl._scope._module) // should use getModule() instead?
        {
    ...
    final static bool semanticTiargs(Loc loc, Scope* sc, Objects* tiargs, int flags)
    {
        ...
        for (size_t j = 0; j < tiargs.dim; j++)
        {
            ...
            Lexpr:
                ...
                {
                    VarDeclaration v;
                    if (ea.op == TOKvar && (v = (cast(VarExp)ea).var.isVarDeclaration()) !is null && !(v.storage_class & STCtemplateparameter))
                    {
    ...
    final bool findBestMatch(Scope* sc, Expressions* fargs)
    {
        ...
        else
        {
            ...
            else if (tdecl && !tdecl.overnext)// Only one template, so we can give better error message
