...
import ddmd.aggregate, ddmd.arraytypes, ddmd.backend, ddmd.complex, ddmd.constfold, ddmd.dclass, ddmd.declaration, ddmd.dinterpret, ddmd.dstruct, ddmd.dtemplate, ddmd.errors, ddmd.expression, ddmd.func, ddmd.globals, ddmd.hdrgen, ddmd.id, ddmd.mtype, ddmd.root.longdouble, ddmd.root.outbuffer, ddmd.root.port, ddmd.root.rmem, ddmd.target, ddmd.tokens, ddmd.utf, ddmd.visitor;
...
extern (C++) bool needToCopyLiteral(Expression expr)
{
    for (;;)
    {
        ...
        case TOKarrayliteral:
            return !(cast(ArrayLiteralExp)expr).ownedByCtfe;
        case TOKassocarrayliteral:
            return !(cast(AssocArrayLiteralExp)expr).ownedByCtfe;
        case TOKstructliteral:
            return !(cast(StructLiteralExp)expr).ownedByCtfe;
        case TOKstring:
        case TOKthis:
        case TOKvar:
            return false;
...
extern (C++) UnionExp copyLiteral(Expression e)
{
    ...
    if (e.op == TOKfunction || e.op == TOKdelegate || e.op == TOKsymoff || e.op == TOKnull || e.op == TOKvar || e.op == TOKdotvar || e.op == TOKint64 || e.op == TOKfloat64 || e.op == TOKchar || e.op == TOKcomplex80 || e.op == TOKvoid || e.op == TOKvector)
    {
        // Simple value types
...
extern (C++) bool isSafePointerCast(Type srcPointee, Type destPointee)
{
    ...
    //     int*     --> uint*
    //     int[5][] --> uint[5][]
    return srcPointee.baseElemOf().isintegral() && destPointee.baseElemOf().isintegral() && srcPointee.size() == destPointee.size();
}
...
extern (C++) bool isCtfeComparable(Expression e)
{
    ...
    if (e.isConst() != 1)
    {
        ...
        if (e.op == TOKsymoff && (cast(SymOffExp)e).var.isTypeInfoDeclaration())
            return true;
        return false;
    }
    return true;
...
extern (C++) int intUnsignedCmp(TOK op, d_uns64 n1, d_uns64 n2)
{
    int n;
    switch (op)
    {
...
extern (C++) Expression ctfeCast(Loc loc, Type type, Type to, Expression e)
{
    ...
    if (e.op == TOKarrayliteral)
        (cast(ArrayLiteralExp)e).ownedByCtfe = true;
    if (e.op == TOKstring)
        (cast(StringExp)e).ownedByCtfe = true;
    return r;
}
...
extern (C++) void recursiveBlockAssign(ArrayLiteralExp ae, Expression val, bool wantRef)
{
    assert(ae.type.ty == Tsarray || ae.type.ty == Tarray);
    Type desttype = (cast(TypeArray)ae.type).next.toBasetype().castMod(0);
    ...
    for (size_t k = 0; k < ae.elements.dim; k++)
    {
        ...
        {
            recursiveBlockAssign(cast(ArrayLiteralExp)(*ae.elements)[k], val, wantRef);
        }
        else
        {
            if (wantRef || cow)
...
extern (C++) bool isCtfeValueValid(Expression newval)
{
    ...
    if (newval.op == TOKarrayliteral)
        return (cast(ArrayLiteralExp)newval).ownedByCtfe;
    if (newval.op == TOKassocarrayliteral)
        return (cast(AssocArrayLiteralExp)newval).ownedByCtfe;
    if (newval.op == TOKstructliteral)
        return (cast(StructLiteralExp)newval).ownedByCtfe;
