...
public:
    ...
    Dsymbols* parseModule()
    {
        ...
        return decldefs;
        Lerr: while (token.value != TOKsemicolon && token.value != TOKeof)
            nextToken();
        nextToken();
        return new Dsymbols();
    }
    ...
    Dsymbols* parseDeclDefs(int once, Dsymbol* pLastDecl = null, PrefixAttributes* pAttrs = null)
    {
        ...
        do
        {
            ...
            default:
                error("declaration expected, not '%s'", token.toChars());
                Lerror: while (token.value != TOKsemicolon && token.value != TOKeof)
                    nextToken();
                nextToken();
                s = null;
                continue;
    ...
    Dsymbol parseCtor(PrefixAttributes* pAttrs)
    {
        ...
        if (token.value == TOKlparen && peekNext() == TOKthis && peekNext2() == TOKrparen)
        {
            ...
            auto f = new PostBlitDeclaration(loc, Loc(), stc, Id._postblit);
            if (pAttrs)
                pAttrs.storageClass = STCundefined;
    ...
    Type parseBasicType()
    {
        ...
        Lident:
            ...
            Lident2: while (token.value == TOKdot)
            {
                nextToken();
                if (token.value != TOKidentifier)
                {
    ...
    L2:
        ...
        while (1)
        {
            ...
            bool isThis = (t.ty == Tident && (cast(TypeIdentifier)t).ident == Id.This);
            if (ident)
        ...
        case TOKforeach_reverse:
            ...
            case TOKif:
                ...
                LagainStc:
                    ...
                    else if (storageClass == 0 && token.value == TOKidentifier && peek(&token).value == TOKsemicolon)
                    {
                        // Check for " ident;"
            ...
            case TOKimport:
                ...
                case TOKtemplate:
                    ...
                    default:
                        ...
                        goto Lerror;
                        Lerror: while (token.value != TOKrcurly && token.value != TOKsemicolon && token.value != TOKeof)
                            nextToken();
                        if (token.value == TOKsemicolon)
                            nextToken();
                        s = null;
...
enum PREC : int
{
    PREC_zero, PREC_expr, PREC_assign, PREC_cond, PREC_oror, PREC_andand, PREC_or, PREC_xor, PREC_and, PREC_equal, PREC_rel, PREC_shift, PREC_add, PREC_mul, PREC_pow, PREC_unary, PREC_primary,
}
