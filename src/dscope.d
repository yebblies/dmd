...
struct Scope
{
    ...
    size_t lastoffset = 0; // offset in docbuf of where to insert next dec (for ditto)
    size_t lastoffset2; // offset in docbuf of where to insert next dec (for unittest)
    OutBuffer* docbuf = null; // buffer for documentation output
    ...
    extern (C++) void mergeCallSuper(Loc loc, uint cs)
    {
        ...
        if (cs != callSuper)
        {
            ...
            bool ok = true;
            if ((aRet && !aAny && bAny) || (bRet && !bAny && aAny))
            {
