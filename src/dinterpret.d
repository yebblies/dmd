...
public:
    ...
    void interpretAssignCommon(BinExp e, fp_t fp, int post = 0)
    {
        ...
        {
            assert((cast(IndexExp)e1).modifiable);
        }
        ...
        if (e1.op == TOKindex && (cast(IndexExp)e1).e1.type.toBasetype().ty == Taarray)
        {
            ...
            if (aggregate.op == TOKassocarrayliteral)
            {
                ...
                Expression index = interpret((cast(IndexExp)e1).e2, istate);
                if (exceptionOrCant(index))
                    return;
                index = resolveSlice(index); // only happens with AA assignment
                AssocArrayLiteralExp existingAA = cast(AssocArrayLiteralExp)aggregate;
                while (depth > 0)
                {
                    ...
                    AssocArrayLiteralExp newAA = cast(AssocArrayLiteralExp)findKeyInAA(e.loc, existingAA, indx);
            ...
            else
            {
                ...
                while (e1.op == TOKindex && (cast(IndexExp)e1).e1.type.toBasetype().ty == Taarray)
                {
                    ...
                    valuesx.push(newval);
                    keysx.push(index);
                    auto newaae = new AssocArrayLiteralExp(e.loc, keysx, valuesx);
                    newaae.ownedByCtfe = true;
                    newaae.type = (cast(IndexExp)e1).e1.type;
                    newval = newaae;
    ...
    Expression interpretAssignToSlice(BinExp e, Expression e1, Expression newval, bool isBlockAssignment)
    {
        ...
        if (aggregate.op == TOKarrayliteral)
        {
            ...
            assert(existingAE.type.ty == Tsarray || existingAE.type.ty == Tarray);
            Type dsttype = (cast(TypeArray)existingAE.type).next.toBasetype().castMod(0);
            ...
            for (size_t j = 0; j < upperbound - lowerbound; j++)
            {
                if (!directblk)
                {
                    ...
                    recursiveBlockAssign(cast(ArrayLiteralExp)(*w)[cast(size_t)(j + firstIndex)], newval, wantRef);
                }
                else
                {
                    if (wantRef || cow)
                        (*existingAE.elements)[cast(size_t)(j + firstIndex)] = newval;
                    else
                        assignInPlace((*existingAE.elements)[cast(size_t)(j + firstIndex)], newval);
                }
            }
            if (!(wantRef || cow) && e.op != TOKblit && e.e2.isLvalue())
            {
                size_t lwr = cast(size_t)firstIndex;
                size_t upr = cast(size_t)(firstIndex + upperbound - lowerbound);
    ...
    void visit(CallExp e)
    {
        ...
        if (ecall.op == TOKdotvar)
        {
            ...
            if (pthis.op == TOKsymoff && (tid = (cast(SymOffExp)pthis).var.isTypeInfoDeclaration()) !is null && tid.tinfo.toBasetype().ty == Tsarray && fd.ident == Id.destroy && e.arguments.dim == 1)
            {
                Type tb = tid.tinfo.baseElemOf();
                if (tb.ty == Tstruct && (cast(TypeStruct)tb).sym.dtor)
                {
                    ...
                    if (ea.op == TOKsymoff)
                        result = getVarExp(e.loc, istate, (cast(SymOffExp)ea).var, ctfeNeedRvalue);
        ...
        if (pthis)
        {
            ...
            {
                VarDeclaration vthis = (cast(SymOffExp)pthis).var.isVarDeclaration();
                assert(vthis);
    ...
    void visit(CatExp e)
    {
        ...
        if (result.op == TOKarrayliteral)
            (cast(ArrayLiteralExp)result).ownedByCtfe = true;
        if (result.op == TOKstring)
            (cast(StringExp)result).ownedByCtfe = true;
    }
    ...
    void visit(PtrExp e)
    {
        ...
        {
            result = interpret((cast(PtrExp)e.e1).e1, istate);
            if (exceptionOrCant(result))
                return;
            if (result.op == TOKnull)
            {
                e.error("null pointer dereference evaluating typeid. '%s' is null", (cast(PtrExp)e.e1).e1.toChars());
                result = CTFEExp.cantexp;
                return;
            }
...
extern (C++) Expression scrubReturnValue(Loc loc, Expression e)
{
    ...
    if (e.op == TOKstring)
    {
        (cast(StringExp)e).ownedByCtfe = false;
    }
    if (e.op == TOKarrayliteral)
    {
        (cast(ArrayLiteralExp)e).ownedByCtfe = false;
