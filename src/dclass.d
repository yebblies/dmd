...
import ddmd.aggregate, ddmd.arraytypes, ddmd.backend, ddmd.clone, ddmd.declaration, ddmd.dmodule, ddmd.doc, ddmd.dscope, ddmd.dsymbol, ddmd.dtemplate, ddmd.errors, ddmd.func, ddmd.globals, ddmd.hdrgen, ddmd.id, ddmd.identifier, ddmd.mtype, ddmd.root.outbuffer, ddmd.root.rmem, ddmd.root.rootobject, ddmd.statement, ddmd.target, ddmd.visitor;
    ...
    extern (C++) bool fillVtbl(ClassDeclaration cd, FuncDeclarations* vtbl, int newinstance)
    {
        ...
        for (size_t j = base.vtblOffset(); j < base.vtbl.dim; j++)
        {
            ...
            if (fd && !fd.isAbstract())
            {
                ...
                if (newinstance && fd.toParent() != cd && ifd.toParent() == base)
...
enum CLASSINFO_SIZE_64 = 0x98;
// value of ClassInfo.size
enum CLASSINFO_SIZE = (0x3C + 12 + 4);
...
public:
    ...
    void semantic(Scope* sc)
    {
        ...
        if (doAncestorsSemantic != SemanticDone)
        {
            ...
            if (baseclasses.dim)
            {
                ...
                baseClass = tc.sym;
                b.base = baseClass;
                if (tc.sym._scope && tc.sym.doAncestorsSemantic != SemanticDone)
    ...
    final Dsymbol search(Loc loc, Identifier ident, int flags = IgnoreNone)
    {
        ...
        if (_scope && doAncestorsSemantic < SemanticDone)
        {
            if (!inuse)
            {
