...
import ddmd.aggregate, ddmd.arraytypes, ddmd.backend, ddmd.dimport, ddmd.dmacro, ddmd.doc, ddmd.dscope, ddmd.dsymbol, ddmd.errors, ddmd.expression, ddmd.globals, ddmd.hdrgen, ddmd.id, ddmd.identifier, ddmd.lexer, ddmd.parse, ddmd.root.file, ddmd.root.filename, ddmd.root.outbuffer, ddmd.root.rmem, ddmd.target, ddmd.visitor;

extern (C++) uint readwordLE(ushort* p)
{
    return ((cast(ubyte*)p)[1] << 8) | (cast(ubyte*)p)[0];
}

extern (C++) uint readwordBE(ushort* p)
{
    return ((cast(ubyte*)p)[0] << 8) | (cast(ubyte*)p)[1];
}

extern (C++) uint readlongLE(uint* p)
{
    return (cast(ubyte*)p)[0] | ((cast(ubyte*)p)[1] << 8) | ((cast(ubyte*)p)[2] << 16) | ((cast(ubyte*)p)[3] << 24);
}

extern (C++) uint readlongBE(uint* p)
{
    return (cast(ubyte*)p)[3] | ((cast(ubyte*)p)[2] << 8) | ((cast(ubyte*)p)[1] << 16) | ((cast(ubyte*)p)[0] << 24);
}
