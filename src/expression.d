 ...
 */
extern (C++) bool arrayExpressionSemantic(Expressions* exps, Scope* sc)
{
    bool err = false;
    if (exps)
    {
...
extern (C++) bool functionParameters(Loc loc, Scope* sc, TypeFunction tf, Type tthis, Expressions* arguments, FuncDeclaration fd, Type* prettype, Expression* peprefix)
{
    ...
    *peprefix = null;
                ...
                case Tarray:
                    {
                        ...
                        auto v = new VarDeclaration(loc, tsa, id, new ExpInitializer(loc, ale));
    ...
    if (tf.linkage == LINKd && tf.varargs == 1)
    {
        ...
        Expression e = createTypeInfoArray(sc, cast(Expression*)&arguments.tdata()[nparams], arguments.dim - nparams);
...
public:
    ...
    Expression syntaxCopy()
    {
        auto exp = new StructLiteralExp(loc, sd, arraySyntaxCopy(elements), stype);
        exp.origin = this;
        return exp;
    }
...
public:
    ...
    /******************************** FuncExp *********************************/
    extern (D) this(Loc loc, FuncLiteralDeclaration fd, TemplateDeclaration td = null)
    {
    ...
    Expression syntaxCopy()
    {
        ...
        if (td)
        {
            td2 = cast(TemplateDeclaration)td.syntaxCopy(null);
        ...
        {
            td2 = null;
            fd2 = cast(FuncLiteralDeclaration)fd.syntaxCopy(null);
        }
        else
        {
...
public:
    ...
    final Expression checkComplexOpAssign(Scope* sc)
    {
        ...
        if (op == TOKmulass || op == TOKdivass || op == TOKmodass)
        {
            ...
            {
                error("%s %s %s is undefined. Did you mean %s %s %s.re ?", e1.type.toChars(), opstr, e2.type.toChars(), e1.type.toChars(), opstr, e2.type.toChars());
                return new ErrorExp();
            }
            else if (e1.type.isimaginary() && e2.type.iscomplex())
            {
                error("%s %s %s is undefined. Did you mean %s %s %s.im ?", e1.type.toChars(), opstr, e2.type.toChars(), e1.type.toChars(), opstr, e2.type.toChars());
                return new ErrorExp();
            }
            else if ((e1.type.isreal() || e1.type.isimaginary()) && e2.type.isimaginary())
            {
                error("%s %s %s is an undefined operation", e1.type.toChars(), opstr, e2.type.toChars());
                return new ErrorExp();
            }
        }
        // generate an error if this is a nonsensical += or -=, eg real += imaginary
        if (op == TOKaddass || op == TOKminass)
        {
            ...
            if ((e1.type.isreal() && (e2.type.isimaginary() || e2.type.iscomplex())) || (e1.type.isimaginary() && (e2.type.isreal() || e2.type.iscomplex())))
            {
                error("%s %s %s is undefined (result is complex)", e1.type.toChars(), Token.toChars(op), e2.type.toChars());
                return new ErrorExp();
            }
...
public:
    ...
    Lagain:
        ...
        if (e1.type)
        {
            ...
            if (t1.ty == Tstruct)
            {
                AggregateDeclaration ad = (cast(TypeStruct)t1).sym;
                if (ad.sizeok == SIZEOKnone)
                {
                    if (ad._scope)
                        ad.semantic(ad._scope);
...
public:
    ...
    Expression semantic(Scope* sc)
    {
        ...
        Type tb1next = tb1.nextOf();
        Type tb2next = tb2.nextOf();
        if (tb1next && tb2next && (tb1next.implicitConvTo(tb2next) >= MATCHconst || tb2next.implicitConvTo(tb1next) >= MATCHconst))
        {
        ...
        else if ((tb1.ty == Tsarray || tb1.ty == Tarray) && e2.implicitConvTo(tb1next) >= MATCHconvert && tb2.ty != Tvoid)
        {
            if (e2.checkPostblit(sc, tb2))
                return new ErrorExp();
        ...
        else if ((tb2.ty == Tsarray || tb2.ty == Tarray) && e1.implicitConvTo(tb2next) >= MATCHconvert && tb1.ty != Tvoid)
        {
 ...
 */
extern (C++) Expression createTypeInfoArray(Scope* sc, Expression* exps, size_t dim)
{
    /*
