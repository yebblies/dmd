...
Module *Module::factory(const char *filename, Identifier *ident, int doDocComment, int doHdrGen)
{
...
void Module::parse()
{
    ...
    const utf8_t *buf = (const utf8_t *)srcfile->buffer;
    ...
    if (buflen >= 2)
    {
        ...
        if (buf[0] == 0xFF && buf[1] == 0xFE)
        {
            ...
            Lutf32:
                ...
                buf = (const utf8_t *) dbuf.extractData();
            }
            else
            {   // UTF-16LE (X86)
                // Convert it to UTF-8
