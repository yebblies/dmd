...
Expression *BinExp::interpretAssignCommon(InterState *istate, CtfeGoal goal, fp_t fp, int post)
{
    ...
    else if (e1->op == TOKdotvar)
    {
        ...
        {
            VarDeclaration *var = exx->op == TOKstructliteral
                ? se->sd->fields[i]
                : ((ClassReferenceExp *)exx)->getFieldAt(i);
            if (var != member &&
                var->offset == member->offset)
            {
                //printf("union overwrite %s %s\n", var->toPrettyChars(), member->toPrettyChars());
                Expression **el = &se->elements->tdata()[fieldi];
