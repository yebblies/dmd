...
void Module::gendocfile()
{
    ...
    DocComment::parseMacros(&escapetable, &macrotable, (const utf8_t *)mbuf.data, mbuf.offset);
    ...
    {   const char *p = toPrettyChars();
        Macro::define(&macrotable, (const utf8_t *)"TITLE", 5, (const utf8_t *)p, strlen(p));
    }
    ...
    {   time_t t;
        ...
        char *p = ctime(&t);
        p = mem.strdup(p);
        Macro::define(&macrotable, (const utf8_t *)"DATETIME", 8, (const utf8_t *)p, strlen(p));
        Macro::define(&macrotable, (const utf8_t *)"YEAR", 4, (const utf8_t *)(p + 20), 4);
    }

    char *srcfilename = srcfile->toChars();
    Macro::define(&macrotable, (const utf8_t *)"SRCFILENAME", 11, (const utf8_t *)srcfilename, strlen(srcfilename));

    char *docfilename = docfile->toChars();
    Macro::define(&macrotable, (const utf8_t *)"DOCFILENAME", 11, (const utf8_t *)docfilename, strlen(docfilename));

    if (dc->copyright)
    {
        dc->copyright->nooutput = 1;
        Macro::define(&macrotable, (const utf8_t *)"COPYRIGHT", 9, (const utf8_t *)dc->copyright->body, dc->copyright->bodylen);
    }
    ...
    //printf("BODY= '%.*s'\n", buf.offset, buf.data);
    Macro::define(&macrotable, (const utf8_t *)"BODY", 4, (const utf8_t *)buf.data, buf.offset);
...
{   size_t length = buf->offset - i;
    const utf8_t *p = (const utf8_t *)&buf->data[i];
    size_t j;
    unsigned sawdot = 0;
...
int isKeyword(const utf8_t *p, size_t len)
{
     ...
     Lcont:
        switch (c)
        {
            ...
            default:
                leadingBlank = 0;
                if (!sc->module->isDocFile &&
                    !inCode && isIdStart((const utf8_t *)&buf->data[i]))
                {
                    ...
                    if (j > i)
                    {
                        ...
                        else
                        {
                            ...
                            else if (isKeyword((const utf8_t *)buf->data + i, j - i))
                            {
                            ...
                            {
                                if (f && isFunctionParameter(f, (const utf8_t *)buf->data + i, j - i))
                                {
...
void highlightCode(Scope *sc, Dsymbol *s, OutBuffer *buf, size_t offset, bool anchor)
{
    ...
    {   const utf8_t c = buf->data[i];
        ...
        else if (isIdStart((const utf8_t *)&buf->data[i]))
        {
            size_t j = skippastident(buf, i);
            if (j > i)
            {
                ...
                {
                    if (isFunctionParameter(f, (const utf8_t *)buf->data + i, j - i))
                    {
                        //printf("highlighting arg '%s', i = %d, j = %d\n", arg->ident->toChars(), i, j);
                        i = buf->bracket(i, "$(DDOC_PARAM ", j, ")") - 1;
                        continue;
...
void highlightCode2(Scope *sc, Dsymbol *s, OutBuffer *buf, size_t offset)
{
    ...
    Lexer lex(NULL, (const utf8_t *)buf->data, 0, buf->offset - 1, 0, 1);
    Token tok;
    OutBuffer res;
    const utf8_t *lastp = (const utf8_t *)buf->data;
