...
struct DocComment
{
    ...
    static DocComment *parse(Scope *sc, Dsymbol *s, const char *comment);
...
int isDitto(const char *comment);
...
Parameter *isFunctionParameter(Dsymbol *s, const char *p, size_t len);

int isIdStart(const char *p);
int isIdTail(const char *p);
int isIndentWS(const char *p);
...
void Module::gendocfile()
{
    ...
    if (!mbuf)             // if not already read the ddoc files
    {   mbuf = new OutBuffer();
    ...
    DocComment::parseMacros(&escapetable, &macrotable, (const char *)mbuf->data, mbuf->offset);

    Scope *sc = Scope::createGlobal(this);      // create root scope
    sc->docbuf = new OutBuffer();
    ...
    {   const char *p = toPrettyChars();
        Macro::define(&macrotable, "TITLE", 5, p, strlen(p));
    }
    ...
    {   time_t t;
        ...
        char *p = ctime(&t);
        p = mem.strdup(p);
        Macro::define(&macrotable, "DATETIME", 8, p, strlen(p));
        Macro::define(&macrotable, "YEAR", 4, p + 20, 4);
    }

    char *srcfilename = srcfile->toChars();
    Macro::define(&macrotable, "SRCFILENAME", 11, srcfilename, strlen(srcfilename));

    char *docfilename = docfile->toChars();
    Macro::define(&macrotable, "DOCFILENAME", 11, docfilename, strlen(docfilename));

    if (dc->copyright)
    {
        dc->copyright->nooutput = 1;
        Macro::define(&macrotable, "COPYRIGHT", 9, dc->copyright->body, dc->copyright->bodylen);
    }

    sc->docbuf->printf("$(DDOC_COMMENT Generated by Ddoc from %s)\n", srcfile->toChars());
    if (isDocFile)
    {
    ...
    //printf("BODY= '%.*s'\n", sc->docbuf->offset, sc->docbuf->data);
    Macro::define(&macrotable, "BODY", 4, (const char *)sc->docbuf->data, sc->docbuf->offset);
     ...
     */
    {
        ...
        for (size_t j = 0; j < buf2.offset; j++)
        {
            ...
            sc->docbuf->writeByte(c);
        }
    }

    // Transfer image to file
    assert(docfile);
    docfile->setbuffer(sc->docbuf->data, sc->docbuf->offset);
    docfile->ref = 1;
...
static bool emitAnchorName(OutBuffer *buf, Dsymbol *s)
{
    ...
    td = s->parent ? s->parent->isTemplateDeclaration() : NULL;
    if (td && td->onemember == s)
        return dot;
    if (dot)
        buf->writeByte('.');
    // Use "this" not "__ctor"
    td = s->isTemplateDeclaration();
    if (s->isCtorDeclaration() || (td &&
...
DocComment *DocComment::parse(Scope *sc, Dsymbol *s, const char *comment)
{
    //printf("parse(%s): '%s'\n", s->toChars(), comment);
...
void DocComment::parseSections(const char *comment)
{   const char *p;
    const char *pstart;
    const char *pend;
    const char *idstart;
...
void DocComment::parseMacros(Escape **pescapetable, Macro **pmacrotable, const char *m, size_t mlen)
{
    const char *p = m;
    size_t len = mlen;
    const char *pend = p + len;
...
void DocComment::parseEscapes(Escape **pescapetable, const char *textstart, size_t textlen)
...
{   size_t length = buf->offset - i;
    const char *p = (const char *)&buf->data[i];
    size_t j;
    unsigned sawdot = 0;
     ...
     Lcont:
        switch (c)
        {
            ...
            default:
                leadingBlank = 0;
                if (!sc->module->isDocFile &&
                    !inCode && isIdStart((const char *)&buf->data[i]))
                {
                    ...
                    if (j > i)
                    {
                        ...
                        else
                        {
                            ...
                            else if (isKeyword((const char *)buf->data + i, j - i))
                            {
                            ...
                            {
                                if (f && isFunctionParameter(f, (const char *)buf->data + i, j - i))
                                {
...
void highlightCode(Scope *sc, Dsymbol *s, OutBuffer *buf, size_t offset, bool anchor)
{
    ...
    {   char c = buf->data[i];
        ...
        else if (isIdStart((const char *)&buf->data[i]))
        {
            size_t j = skippastident(buf, i);
            if (j > i)
            {
                ...
                {
                    if (isFunctionParameter(f, (const char *)buf->data + i, j - i))
                    {
                        //printf("highlighting arg '%s', i = %d, j = %d\n", arg->ident->toChars(), i, j);
                        i = buf->bracket(i, "$(DDOC_PARAM ", j, ")") - 1;
                        continue;
...
void highlightCode2(Scope *sc, Dsymbol *s, OutBuffer *buf, size_t offset)
{
    ...
    Lexer lex(NULL, (const char *)buf->data, 0, buf->offset - 1, 0, 1);
    Token tok;
    OutBuffer res;
    const char *lastp = (const char *)buf->data;
...
const char *Escape::escapeChar(char c)
{
#if 1
    printf("0x%.8X\n", c);
    assert(c < 256);
