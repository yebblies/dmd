...
public:
    ...
    void semantic(Scope* sc)
    {
        ...
        if (ClassDeclaration cd = parent.isClassDeclaration())
        {
            ...
            for (size_t i = 0; i < cd.interfaces_dim; i++)
            {
                ...
                vi = findVtblIndex(cast(Dsymbols*)&b.base.vtbl, cast(int)b.base.vtbl.dim);
                switch (vi)
                {
                case -1:
                    break;
    ...
    Ldone:
         ...
         */
        TemplateInstance ti;
        if (fbody && (isFuncLiteralDeclaration() || isInstantiated() && !isVirtualMethod() && !(ti = parent.isTemplateInstance(), ti && !ti.isTemplateMixin() && ti.tempdecl.ident != ident)))
        {
    ...
    final void semantic3(Scope* sc)
    {
        ...
        if (fbody || frequire || fensure)
        {
            ...
            if (addPreInvariant())
            {
                Expression e = addInvariant(sc, ad, vthis, isDtorDeclaration() !is null);
                if (e)
            ...
            if (addPostInvariant())
            {
                Expression e = addInvariant(sc, ad, vthis, isCtorDeclaration() !is null);
                if (e)
            ...
            else
            {
                ...
                if (_arguments)
                {
                    version (GNU)
                    {
                        ...
                        v_arguments_var._init = new VoidInitializer(loc);
                    }
 ...
 */
extern (C++) Expression addInvariant(Scope* sc, AggregateDeclaration ad, VarDeclaration vthis, bool direct)
{
    Expression e = null;
    if (direct)
    {
...
public:
    ...
    void semantic(Scope* sc)
    {
         ...
         */
        if (ad && tf.varargs == 0 && Parameter.dim(tf.parameters) == 0 && (!this.parent.isTemplateInstance() || this.parent.isTemplateMixin()))
        {
