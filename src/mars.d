...
import ddmd.arraytypes, ddmd.backend, ddmd.builtin, ddmd.cond, ddmd.dinifile, ddmd.dinterpret, ddmd.dmodule, ddmd.doc, ddmd.dscope, ddmd.dsymbol, ddmd.dunittest, ddmd.errors, ddmd.expression, ddmd.globals, ddmd.hdrgen, ddmd.id, ddmd.identifier, ddmd.inline, ddmd.json, ddmd.lexer, ddmd.lib, ddmd.link, ddmd.mtype, ddmd.parse, ddmd.root.file, ddmd.root.filename, ddmd.root.man, ddmd.root.outbuffer, ddmd.root.response, ddmd.root.rmem, ddmd.target, ddmd.tokens, ddmd.traits;
...
extern (C++) int tryMain(size_t argc, const(char)** argv)
{
    ...
    parseConfFile(global.inifilename, envsection.ptr);
 ...
 */
extern (C++) void getenv_setargv(const(char)* envvar, size_t* pargc, const(char)*** pargv)
{
    char* p;
    int instring;
    int slash;
 ...
 */
extern (C++) static const(char)* parse_arch_arg(size_t argc, const(char)** argv, const(char)* arch)
{
    for (size_t i = 0; i < argc; ++i)
    {
        const(char)* p = argv[i];
 ...
 */
extern (C++) static const(char)* parse_conf_arg(size_t argc, const(char)** argv)
{
...
extern (C++) Symbols* Symbols_create()
{
    return new Symbols();
}
