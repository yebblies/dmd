...
void scanMachObjModule(void* pctx, void (*pAddSymbol)(void* pctx, char* name, int pickAny), void *base, size_t buflen, const char *module_name, Loc loc)
{
    ...
    if (header->magic == MH_MAGIC)
    {
        ...
        if (buflen < sizeof(mach_header) + header->sizeofcmds)
        {   reason = __LINE__;
            goto Lcorrupt;
        }
        ncmds = header->ncmds;
    ...
    {
        header64 = (mach_header_64 *)buf;
        if (buflen < sizeof(mach_header_64))
            goto Lcorrupt;
        ...
        if (buflen < sizeof(mach_header_64) + header64->sizeofcmds)
        {   reason = __LINE__;
            goto Lcorrupt;
        }
    ...
    {   load_command *command = (load_command *)commands;
        //printf("cmd = 0x%02x, cmdsize = %u\n", command->cmd, command->cmdsize);
        switch (command->cmd)
        {
            case LC_SEGMENT:
                segment_commands = (segment_command *)command;
                break;
            case LC_SEGMENT_64:
                segment_commands64 = (segment_command_64 *)command;
                break;
            case LC_SYMTAB:
                symtab_commands = (symtab_command *)command;
                break;
            case LC_DYSYMTAB:
                dysymtab_commands = (dysymtab_command *)command;
                break;
        }
    ...
    if (symtab_commands)
    {
        ...
        if (header->magic == MH_MAGIC_64)
        {
            ...
            nlist_64 *symtab = (nlist_64 *)((char *)buf + symtab_commands->symoff);
            if (buflen < symtab_commands->symoff + symtab_commands->nsyms * sizeof(nlist_64))
            {   reason = __LINE__;
                goto Lcorrupt;
            }
        ...
        {
            // Get pointer to symbol table
            nlist *symtab = (nlist *)((char *)buf + symtab_commands->symoff);
            if (buflen < symtab_commands->symoff + symtab_commands->nsyms * sizeof(nlist))
            {   reason = __LINE__;
                goto Lcorrupt;
            }
