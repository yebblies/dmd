...
import ddmd.aggregate, ddmd.arraytypes, ddmd.cond, ddmd.dclass, ddmd.declaration, ddmd.dinterpret, ddmd.dmodule, ddmd.dscope, ddmd.dstruct, ddmd.dsymbol, ddmd.dtemplate, ddmd.errors, ddmd.expression, ddmd.func, ddmd.globals, ddmd.hdrgen, ddmd.id, ddmd.identifier, ddmd.mars, ddmd.parse, ddmd.root.outbuffer, ddmd.root.rmem, ddmd.tokens, ddmd.utf, ddmd.visitor;
...
public:
     ...
     */
    final static Scope* createNewScope(Scope* sc, StorageClass stc, LINK linkage, Prot protection, int explicitProtection, structalign_t structalign)
    {
        Scope* sc2 = sc;
        if (stc != sc.stc || linkage != sc.linkage || !protection.isSubsetOf(sc.protection) || explicitProtection != sc.explicitProtection || structalign != sc.structalign)
        {
...
public:
     ...
     */
    final static const(char)* stcToChars(char* tmp, ref StorageClass stc)
    {
        struct SCstring
        {
            StorageClass stc;
    ...
    final static void stcToCBuffer(OutBuffer* buf, StorageClass stc)
    {
        while (stc)
        {
            const(size_t) BUFFER_LEN = 20;
            char[BUFFER_LEN] tmp;
            const(char)* p = stcToChars(tmp.ptr, stc);
            if (!p)
                break;
