...
extern (C++) Expression semanticTraits(TraitsExp e, Scope* sc)
{
    ...
    else if (e.ident == Id.allMembers || e.ident == Id.derivedMembers)
    {
        ...
        Import _import;
        if ((_import = s.isImport()) !is null)
        {
            // Bugzilla 9692
            sds = _import.mod;
        }
        else if ((sds = s.isScopeDsymbol()) is null)
        {
        ...
        struct PushIdentsDg
        {
            extern (C++) static int dg(void* ctx, size_t n, Dsymbol sm)
            {
                ...
                if (sm.ident)
                {
                    if (sm.ident != Id.ctor && sm.ident != Id.dtor && sm.ident != Id._postblit && memcmp(sm.ident.string, cast(char*)"__", 2) == 0)
                    {
                        return 0;
                    }
                    //printf("\t%s\n", sm->ident->toChars());
