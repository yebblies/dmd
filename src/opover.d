...
import ddmd.aggregate, ddmd.arraytypes, ddmd.dclass, ddmd.declaration, ddmd.dscope, ddmd.dstruct, ddmd.dsymbol, ddmd.dtemplate, ddmd.errors, ddmd.expression, ddmd.func, ddmd.globals, ddmd.id, ddmd.identifier, ddmd.mtype, ddmd.statement, ddmd.tokens, ddmd.visitor;
...
extern (C++) bool inferAggregate(ForeachStatement fes, Scope* sc, ref Dsymbol sapply)
{
    ...
    while (1)
    {
        ...
        case Tdelegate:
            ...
            {
                DelegateExp de = cast(DelegateExp)fes.aggr;
...
version (none)
{
     ...
     */
    extern (C++) void inferApplyArgTypesZ(TemplateDeclaration tstart, Parameters* parameters)
    {
        for (TemplateDeclaration td = tstart; td; td = td.overnext)
        {
            ...
            if (!tap || !tap.specType || tap.specType.ty != Tfunction)
                continue;
            TypeFunction tf = cast(TypeFunction)tap.specType;
